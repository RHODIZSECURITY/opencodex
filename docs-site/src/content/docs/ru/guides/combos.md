...
Исключение для завершающих ошибок 400: структурированный отказ от `user` или неподдерживаемое значение `reasoning.effort`/`reasoning_effort` позволяет до начала вывода перейти к следующей допустимой цели без периода охлаждения. Отказ политики безопасности, отмена и уже начавшийся вывод по-прежнему запрещают повторное выполнение.
