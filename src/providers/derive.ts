...
function enrichReasoningMetadataByDestination(prov: OcxProviderConfig): void {
  ...
  // A custom/case-changed row on a known fixed endpoint must inherit the same
  // stateless thinking replay contract. Explicit [] remains an operator opt-out.
  if (!prov.preserveReasoningContentModels && destination?.preserveReasoningContentModels) {
    prov.preserveReasoningContentModels = [...destination.preserveReasoningContentModels];
  }
  if (!prov.requiresReasoningPlaceholderModels && destination?.requiresReasoningPlaceholderModels) {
    prov.requiresReasoningPlaceholderModels = [...destination.requiresReasoningPlaceholderModels];
  }
  if (!prov.autoToolChoiceOnlyModels && destination?.autoToolChoiceOnlyModels) {
    prov.autoToolChoiceOnlyModels = [...destination.autoToolChoiceOnlyModels];
  }
}
